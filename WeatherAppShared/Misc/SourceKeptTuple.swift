...
public typealias SourceKeptTuple<T, U> = (source: T, result: U)
public func keepSource<T, U>(_ transform: @escaping (T) -> U) -> (T) -> SourceKeptTuple<T, U> {
  return { source in SourceKeptTuple(source: source, result: transform(source)) }
}

public func transformResult<T, U, V>(
...
public func transformResult<T, U, V, Error>(
